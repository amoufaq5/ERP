...
export const GET = withAuthAndTenantParams<{ params: Promise<{ id: string }> }>(
  ...
  try {
    ...
    const record = await db.journalEntry.findFirst({
      where: { id },
      include: {
...
export const PATCH = withAuthAndTenantParams(async (req: NextRequest, { params }: { params: Promise<{ id: string }> }, { db, tenantId, role, userId }) => {
  try {
    ...
    const existing = await db.journalEntry.findFirst({ where: { id } });
    ...
    if (lines && Array.isArray(lines)) {
      ...
      await db.journalEntryLine.deleteMany({ where: { journalEntryId: id } });
      data.lines = {
