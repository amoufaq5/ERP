...
export default function ReturnsPage() {
  ...
  const approvedValue = returns.filter(r => r.status === "Approved" || r.status === "Credit Issued").reduce((s, r) => s + r.value, 0);
  const returnRate = totalValue > 0 ? ((returns.length / (returns.length + 500)) * 100).toFixed(1) : "0.0"; // approximate rate against estimated total orders
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <StatsCard icon={DollarSign} title="Approved Value" value={fmt(approvedValue)} iconColor="bg-green-100 text-green-700" />
        <StatsCard icon={TrendingDown} title="Return Rate" value={`${returnRate}%`} subtitle="Of total sales (estimated)" iconColor="bg-red-100 text-red-700" />
      </div>
